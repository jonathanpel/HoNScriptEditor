 ...
 * is read only.
 */
final class HoNShareableInputSource extends InputSource {
    ...
    public static HoNShareableInputSource create(InputSource peer) {
        if (peer == null) throw new NullPointerException();
        if (peer instanceof HoNShareableInputSource) {
            return (HoNShareableInputSource) peer;
        } else {
            return new HoNShareableInputSource(peer, BUFFER_SIZE);
        }
    }

    private HoNShareableInputSource(InputSource peer, int bufferSize) {
        this.peer = peer;
