...
import org.hon.xml.completion.CompletionPaintComponent.ElementPaintComponent;
...
public class ElementResultItem extends CompletionResultItem {
    ...
    public String getReplacementText() {
        ...
        for (AbstractAttribute aa : element.getAttributes()) {
            ...
            if (a.getUse() == Use.REQUIRED) {
                ...
                if (firstAttr) {
                    caretPosition = CompletionUtil.TAG_FIRST_CHAR.length() + itemText.length() + buffer.length() - 1;
                    firstAttr = false;
                }                
            }
        }
        replacingText = 
			...
			(element.getChildElements().isEmpty() ? CompletionUtil.END_TAG_SUFFIX : CompletionUtil.TAG_LAST_CHAR));
    ...
    public int getCaretPosition() {
		...
		return ((caretPosition > 0 ? caretPosition : replacingText.length()));
    }    
}
